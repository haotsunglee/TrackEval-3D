...
class MotChallenge2DBox(_BaseDataset):
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                    ...
                    if is_gt:
                        ...
                        raise TrackEvalException(
                            'Cannot convert tracking data from tracker %s, sequence %s to float. Is data corrupted?' % (
                                tracker, seq))
                try:
                    raw_data['dets'][t] = np.atleast_2d(time_data[:, 2:26])
                ...
                if time_data.shape[1] >= 8:
                    raw_data['classes'][t] = np.atleast_1d(time_data[:, 27]).astype(int)
                else:
                    if not is_gt:
                        ...
                        raise TrackEvalException(
                            'GT data is not in a valid format, there is not enough rows in seq %s, timestep %i.' % (
                                seq, t))
                if is_gt:
                    gt_extras_dict = {'zero_marked': np.atleast_1d(time_data[:, 26].astype(int))}
                    raw_data['gt_extras'][t] = gt_extras_dict
                else:
                    raw_data['tracker_confidences'][t] = np.atleast_1d(time_data[:, 26])
            else:
                raw_data['dets'][t] = np.empty((0, 24))
            ...
            if is_gt:
                raw_data['gt_crowd_ignore_regions'][t] = np.empty((0, 24))
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        similarity_scores = self._calculate_box_ious_3D(gt_dets_t, tracker_dets_t)
