...
class _BaseDataset(ABC):
    ...
    def get_raw_seq_data(self, tracker, seq):
        ...
        for t, (gt_dets_t, tracker_dets_t) in enumerate(zip(raw_data['gt_dets'], raw_data['tracker_dets'])):
            ...
            # print(ious.shape) # (len(gt_dets_t), len(tracker_dets_t))
    ...
    def Cal_area_2poly(self, data1,data2):
    ...
    def _box3d_iou(self, corners1, corners2):
    ...
    def _calculate_box_ious_3D(self, bboxes1, bboxes2):
        ...
        for i in range(dim1):
            for j in range(dim2):
                iou = self._box3d_iou(corners_1[i], corners_2[j])
                ious[i][j] = iou
        return ious
